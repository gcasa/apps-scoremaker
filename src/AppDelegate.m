                    ...
                    NSTextAlignment alignment)
{
  ...
  NSDictionary *attributes = [NSDictionary
    dictionaryWithObjectsAndKeys:font, NSFontAttributeName, color, NSForegroundColorAttributeName,
...
- (void)drawRect:(NSRect)dirtyRect
{
  ...
  NSArray *notes =
    [NSArray arrayWithObjects:[NSValue valueWithPoint:NSMakePoint (328.0, 112.0)],
  ...
  NSArray *noteColors = [NSArray
    arrayWithObjects:[NSColor colorWithCalibratedRed:0.96 green:0.36 blue:0.28 alpha:1.0],
  ...
  for (NSUInteger index = 0; index < [notes count]; index++)
    {
      ...
      [color setFill];
      NSBezierPath *head =
        [NSBezierPath bezierPathWithOvalInRect:NSMakeRect (point.x, point.y, 24.0, 16.0)];
  ...
  ScoreMakerDrawText (@"MAKER", NSMakeRect (44.0, 89.0, 230.0, 54.0),
                      [NSFont boldSystemFontOfSize:46.0], paper, NSTextAlignmentLeft);
  ScoreMakerDrawText (@"COMPOSE  ·  ENGRAVE  ·  PLAY", NSMakeRect (47.0, 161.0, 260.0, 22.0),
  ...
  ScoreMakerDrawText (@"Shape notation, sculpt sound, and hear every phrase take flight — from the "
                      @"first mark on the staff to the final resonance.",
                      NSMakeRect (44.0, 326.0, 510.0, 52.0), [NSFont systemFontOfSize:14.0],
  ...
  ScoreMakerDrawText (
    @"VERSION", NSMakeRect (44.0, 430.0, 80.0, 18.0), [NSFont boldSystemFontOfSize:10.0],
  ...
  ScoreMakerDrawText (
    @"CRAFT", NSMakeRect (178.0, 430.0, 80.0, 18.0), [NSFont boldSystemFontOfSize:10.0],
    [NSColor colorWithCalibratedRed:0.55 green:0.27 blue:0.22 alpha:1.0], NSTextAlignmentLeft);
  ScoreMakerDrawText (@"Native AppKit", NSMakeRect (178.0, 450.0, 130.0, 22.0),
                      [NSFont systemFontOfSize:14.0], ink, NSTextAlignmentLeft);
  ScoreMakerDrawText (@"♪", NSMakeRect (535.0, 426.0, 40.0, 50.0), [NSFont systemFontOfSize:34.0],
  ...
  ScoreMakerDrawText (@"Designed for musicians who think in color, gesture, and time.",
                      ...
                      [[NSFontManager sharedFontManager] convertFont:[NSFont systemFontOfSize:12.0]
...
- (id)init
{
  ...
  if (self)
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector (scorefileConsoleDidPrint:)
                                                 name:ScorefileConsoleDidPrintNotification
                                               object:nil];
  return self;
}
...
{
  (void)notification;
  _applicationDidFinishLaunching = YES;
  _playlistPaths = [[NSMutableArray alloc] init];
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector (playlistDocumentDidFinish:)
                                               name:ScoreMakerDocumentPlaybackDidFinishNotification
                                             object:nil];
  [self buildMenu];
  NSString *consoleOutput = [ScorefileParser consoleOutput];
  if ([consoleOutput length] > 0)
    {
      [self createConsolePanelIfNeeded];
      [_consoleTextView setString:consoleOutput];
      [_consolePanel makeKeyAndOrderFront:self];
    }
}
...
#if defined(__APPLE__)
  [controller
    openDocumentWithContentsOfURL:[NSURL fileURLWithPath:path]
                          display:YES
...
- (void)createConsolePanelIfNeeded
{
  ...
  NSRect frame = NSMakeRect (0.0, 0.0, 680.0, 360.0);
  _consolePanel = [[NSPanel alloc] initWithContentRect:frame
                                             styleMask:ScoreMakerPanelStyle
                                               backing:NSBackingStoreBuffered
                                                 defer:NO];
  [_consolePanel setTitle:@"Console"];
  [_consolePanel setReleasedWhenClosed:NO];
  ...
  _consoleTextView = [[NSTextView alloc] initWithFrame:frame];
  [_consoleTextView setEditable:NO];
  [_consoleTextView setSelectable:YES];
  [_consoleTextView setRichText:NO];
  [_consoleTextView setFont:[NSFont userFixedPitchFontOfSize:12.0]];
  [_consoleTextView setAutoresizingMask:NSViewWidthSizable];
  [[_consoleTextView textContainer] setContainerSize:NSMakeSize (FLT_MAX, FLT_MAX)];
  [[_consoleTextView textContainer] setWidthTracksTextView:NO];
  [scroll setDocumentView:_consoleTextView];
  [_consolePanel setContentView:scroll];
  [_consolePanel center];
}
...
- (void)showConsole:(id)sender
{
  (void)sender;
  [self createConsolePanelIfNeeded];
  [_consoleTextView setString:[ScorefileParser consoleOutput]];
  [_consolePanel makeKeyAndOrderFront:self];
}
...
{
  (void)sender;
  [ScorefileParser clearConsoleOutput];
  [self createConsolePanelIfNeeded];
  [[_consoleTextView textStorage]
    setAttributedString:[[[NSAttributedString alloc] initWithString:@""] autorelease]];
}

- (void)scorefileConsoleDidPrint:(NSNotification *)notification
{
  if (![NSThread isMainThread])
    {
      [self performSelectorOnMainThread:@selector (scorefileConsoleDidPrint:)
                             withObject:notification
                          waitUntilDone:NO];
      return;
    }

  NSString *line = [[notification userInfo] objectForKey:ScorefileConsoleLineKey];
  ...
  if (!_playlistPanel)
    {
      ...
      NSScrollView *scroll =
        [[[NSScrollView alloc] initWithFrame:NSMakeRect (20.0, 92.0, 520.0, 290.0)] autorelease];
      [scroll setHasVerticalScroller:YES];
      ...
      [content addSubview:[self playlistButtonWithTitle:@"Add..."
                                                  frame:NSMakeRect (20, 52, 72, 28)
                                                 action:@selector (addPlaylistItems:)]];
      [content addSubview:[self playlistButtonWithTitle:@"Remove"
                                                  frame:NSMakeRect (98, 52, 72, 28)
                                                 action:@selector (removePlaylistItem:)]];
      [content addSubview:[self playlistButtonWithTitle:@"Up"
                                                  frame:NSMakeRect (176, 52, 54, 28)
                                                 action:@selector (movePlaylistItemUp:)]];
      [content addSubview:[self playlistButtonWithTitle:@"Down"
                                                  frame:NSMakeRect (236, 52, 58, 28)
                                                 action:@selector (movePlaylistItemDown:)]];
      NSTextField *delayLabel =
        [[[NSTextField alloc] initWithFrame:NSMakeRect (310, 57, 105, 20)] autorelease];
      [delayLabel setStringValue:@"Delay (seconds):"];
      [delayLabel setEditable:NO];
      [delayLabel setBordered:NO];
      ...
      [content addSubview:[self playlistButtonWithTitle:@"Play"
                                                  frame:NSMakeRect (20, 14, 72, 28)
                                                 action:@selector (playPlaylist:)]];
      [content addSubview:[self playlistButtonWithTitle:@"Stop"
                                                  frame:NSMakeRect (98, 14, 72, 28)
                                                 action:@selector (stopPlaylist:)]];
      [content addSubview:[self playlistButtonWithTitle:@"Save..."
                                                  frame:NSMakeRect (176, 14, 72, 28)
                                                 action:@selector (savePlaylist:)]];
      [content addSubview:[self playlistButtonWithTitle:@"Load..."
                                                  frame:NSMakeRect (254, 14, 72, 28)
      ...
      [_playlistStatusField setEditable:NO];
      [_playlistStatusField setBordered:NO];
...
#pragma clang diagnostic pop
  ...
  NSArray *items =
    [object isKindOfClass:[NSDictionary class]] ? [object objectForKey:@"items"] : nil;
  NSNumber *version =
    [object isKindOfClass:[NSDictionary class]] ? [object objectForKey:@"version"] : nil;
  NSNumber *delay =
    [object isKindOfClass:[NSDictionary class]] ? [object objectForKey:@"delay"] : nil;
  ...
  if (!valid)
    {
      if (!error)
        error = [NSError errorWithDomain:@"ScoreMakerPlaylistError"
                                    code:1
                                userInfo:@{
...
- (void)removePlaylistItem:(id)sender
{
  ...
  if (row >= 0 && (NSUInteger)row < [_playlistPaths count])
    {
      [_playlistPaths removeObjectAtIndex:(NSUInteger)row];
      [_playlistTable reloadData];
    }
}
...
- (void)movePlaylistItemBy:(NSInteger)offset
{
  ...
  if (row < 0 || target < 0 || (NSUInteger)target >= [_playlistPaths count])
    return;
...
- (void)movePlaylistItemUp:(id)sender
{
  (void)sender;
  [self movePlaylistItemBy:-1];
}

- (void)movePlaylistItemDown:(id)sender
{
  (void)sender;
  [self movePlaylistItemBy:1];
}
...
{
  if (!_playlistTable || _playlistIndex >= [_playlistPaths count])
    return;
...
- (void)playPlaylistItem
{
  ...
  [_playlistStatusField
    setStringValue:[NSString stringWithFormat:@"Playing %lu of %lu: %@",
                                              (unsigned long)(_playlistIndex + 1),
                                              (unsigned long)[_playlistPaths count],
                  ...
                  if (_playlistPlaying && [document isKindOfClass:[ScoreMakerDocument class]])
                    {
                      ...
                      [[(ScoreMakerDocument *)document scoreView] selectNote:nil
                                                             scrollToVisible:NO];
...
- (void)playlistDocumentDidFinish:(NSNotification *)notification
{
  ...
  if (finished
      && ![[expected stringByStandardizingPath]
        isEqualToString:[finished stringByStandardizingPath]])
    return;
  [_playlistTable deselectAll:self];
  _playlistIndex++;
  if (_playlistIndex >= [_playlistPaths count])
    {
      [self stopPlaylist:nil];
      return;
    }
  ...
  [_playlistStatusField
    setStringValue:[NSString stringWithFormat:@"Next track in %.1f seconds", delay]];
  _playlistDelayTimer = [[NSTimer scheduledTimerWithTimeInterval:MAX (0.001, delay)
                                                          target:self
                                                        selector:@selector (playlistDelayDidFire:)
                                                        userInfo:nil
                                                         repeats:NO] retain];
}
...
- (void)buildMenu
{
...
#if !defined(__APPLE__)
  NSMenuItem *appItem = [[[NSMenuItem alloc] initWithTitle:@"Info" action:NULL
                                             keyEquivalent:@""] autorelease];
  ...
  [fileMenu addItem:[[[NSMenuItem alloc] initWithTitle:@"Save As..."
                                         ...
                                         keyEquivalent:@"S"] autorelease]];
  [fileMenu addItem:[[[NSMenuItem alloc] initWithTitle:@"Revert to Saved"
  ...
  for (NSNumber *percent in @[ @50, @75, @100, @125, @150, @200 ])
    {
      NSMenuItem *zoom =
        [[[NSMenuItem alloc] initWithTitle:[NSString stringWithFormat:@"%@%%", percent]
                                    action:@selector (setScoreZoom:)
                             keyEquivalent:@""] autorelease];
  ...
  [viewMenu addItem:[NSMenuItem separatorItem]];
  NSMenuItem *consoleItem = [[[NSMenuItem alloc] initWithTitle:@"Console"
                                                        action:@selector (showConsole:)
                                                 keyEquivalent:@""] autorelease];
  [consoleItem setTarget:self];
  [viewMenu addItem:consoleItem];
  NSMenuItem *clearConsoleItem = [[[NSMenuItem alloc] initWithTitle:@"Clear Console"
                                                             action:@selector (clearConsole:)
                                                      keyEquivalent:@""] autorelease];
  [clearConsoleItem setTarget:self];
  [viewMenu addItem:clearConsoleItem];
  ...
  for (NSString *name in
       @[ @"Piano", @"Choir (SATB)", @"String Quartet", @"Concert Band", @"Orchestra" ])
    {
  ...
  [scoreMenu addItem:[[[NSMenuItem alloc] initWithTitle:@"Relink or Substitute Audio Unit..."
                                          ...
                                          keyEquivalent:@""] autorelease]];
  [scoreMenu
    addItem:[[[NSMenuItem alloc] initWithTitle:@"Audio Unit Compatibility Report..."
